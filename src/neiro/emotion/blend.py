...
weights directly from each new `NeiroState` makes the face *snap*
...
class ExpressionBlender:
    """Turns a stream of `NeiroState` into a stream of VRM weight maps.
    ...
    def target_weights(self, state: NeiroState) -> dict[str, float]:
    ...
    def step(self, state: NeiroState, dt: float) -> dict[str, float]:
    ...
    def settle(
        self, state: NeiroState, seconds: float = 2.0, fps: float = 60.0
    ) -> dict[str, float]:
