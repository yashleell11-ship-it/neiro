...
**Barge-in (Stage 2).** While Neiro is speaking, a much stricter gate:
...
class SileroVad:
    ...
    def __init__(self, cfg: Neiro | None = None, model_path: Path | None = None) -> None:
        ...
        if not Path(path).exists():
            raise FileNotFoundError(
                ...
                "`neiro fetch-models --only silero-vad`."
            )
...
class BargeInDetector:
    ...
    cuts Neiro off mid-sentence and a false negative just means he has to
    ...
    def start_speaking(self) -> None:
        """Neiro has begun to talk. Re-arms the dead zone."""
