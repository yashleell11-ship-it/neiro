"""Route Neiro's audio to a NAMED PipeWire node, never 'default'.
...
def suppress_alsa_errors() -> None:
    ...
    ALSA's own device-probing, not from anything Neiro does.
    """
...
def resolve_active_profile(cfg: Neiro | None = None) -> ResolvedDevice:
    cfg = cfg or Neiro()
    name = cfg.audio.active_profile
    if not name or name not in cfg.audio.profiles:
        raise NoActiveProfileError(
            "No active audio profile configured. Run: neiro doctor --audio-inventory, "
            "then set [audio] active_profile and the matching "
            "[audio.profiles.<name>] table in ~/.config/neiro/config.toml."
        )
