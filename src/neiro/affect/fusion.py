...
from neiro.affect.baseline import _mad, _median
...
class LaneBCalibration:
    ...
    def _z(self, value: float, samples: deque[float], cfg: Neiro) -> float | None:
        if not samples:
            return None
        values = list(samples)
    ...
    def to_z(
        self, arousal: float, valence: float, cfg: Neiro | None = None
