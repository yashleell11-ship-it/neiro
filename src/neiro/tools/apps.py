...
1. This never touches Hyprland's socket at all. Local launches go
   ...
   entirely from Neiro's own table, over the same SSH key already used
   for this machine's training operations.
2. `app` is a `Literal` the registry resolves against a table defined
   here, in Neiro's own code (see registry.py's `_field_is_safe`). The
...
BOX_KEY = str(Path.home() / ".ssh" / "neiro_box_ed25519")
SSH_EXE = "ssh"
SSH_TIMEOUT_S = 15.0
...
def open_local(app: App) -> str:
    ...
    try:
        # start_new_session so the app outlives Neiro's own process tree,
        # same as any launcher — closing Neiro should not close Discord.
...
def open_on_box(app: App) -> str:
    ...
    try:
        result = subprocess.run(
            [
                SSH_EXE, "-o", "BatchMode=yes", "-o", "ConnectTimeout=8",
