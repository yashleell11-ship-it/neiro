...
def run(name: str, count: int, cfg: Neiro | None = None) -> int:
    cfg = cfg or Neiro()
    console = Console()

    if not sys.stdin.isatty():
        console.print(
            "[red]neiro record-set needs a real interactive terminal[/red] — it "
    ...
    console.print(
        f"\nScore the current STT against them with:  [bold]neiro wer --name {name}[/bold]"
    )
    return 0
