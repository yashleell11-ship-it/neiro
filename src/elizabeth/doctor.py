...
from elizabeth.llm.ollama_native import THINKING_FIELD
from elizabeth.llm.openai_compat import REASONING_FIELDS, THINK_SNIFF_CHARS
...
MODEL_COMPLETE_MARKER = ".elizabeth-complete"
...
def audio_inventory() -> int:
    """`elizabeth doctor --audio-inventory` — Stage 0 Task 0.
    ...
    console.print(
        ...
        r"~/.config/elizabeth/config.toml under \[audio.profiles.earbuds] and "
...
def check_ct2_compute_type(supported: Iterable[str] | None, wanted: str, error: str = "") -> Check:
    ...
    return Check(
        ...
        remedy=None
        if ok
        else "set stt.compute_type in ~/.config/elizabeth/config.toml to one that is offered, "
...
def check_llm(probe: LlmProbe, base_url: str) -> Check:
    ...
    if probe.server is None:
        return Check(
            ...
            "--reasoning-format none`, or point llm.base_url in ~/.config/elizabeth/config.toml at it",
        )
    if probe.error:
        ...
        return Check(
            ...
            "llm.model in ~/.config/elizabeth/config.toml to a name it shows"
            if model_missing
...
def check_profile_node(
    ...
    if not profile:
        return Check(
            name, False, "no active profile", remedy="run: elizabeth doctor --audio-inventory"
        )
    if not wanted:
        return Check(
            name,
            ...
            remedy="set it to a node name from: elizabeth doctor --audio-inventory",
        )
    ...
    if node is None:
        return Check(
            ...
            "nodes are live — elizabeth doctor --audio-inventory shows them",
        )
    if node.muted:
        return Check(
            name,
...
def _model_remedy(name: str, problem: str) -> str:
    ...
    if problem == NOT_DOWNLOADED:
        return "uv run elizabeth fetch-models --runs local --purpose runtime"
    if problem == NO_RECEIPTS:
        return (
            ...
            f"uv run elizabeth fetch-models --only {name} --force"
        )
    return (
        f"rm the named file under models/{name}/, then: "
        f"uv run elizabeth fetch-models --only {name} --force  (HF re-pulls only what is missing)"
    )
...
def _check_vad_frame_size(cfg) -> Check:
    ...
    if cfg is None:
        return Check(
            ...
            False,
            "config unreadable",
            remedy="check ~/.config/elizabeth/config.toml",
        )
...
def _check_prompt() -> Check:
    ...
    try:
        from elizabeth.llm.prompt import PROMPT_VERSION, load_prompt, prompt_fingerprint
...
def _check_profiles_named(cfg) -> Check:
    ...
    return Check(
        ...
        remedy=None if has_profiles else "run: elizabeth doctor --audio-inventory",
    )
...
def _load_config():
    """`Elizabeth()` or None. A malformed config.toml can fail in several
...
def _vram_need_mib() -> int:
    from elizabeth.modelspec import select
...
def _observe_ws() -> WsOutcome:
    ...
    from elizabeth.server import TOKEN_SUBPROTOCOL_PREFIX, Session, build_app
    ...
    code: int | None = None
    try:
        with client.websocket_connect("/elizabeth", headers=origin):
            accepted = True
    ...
    try:
        with client.websocket_connect(
            "/elizabeth", subprotocols=[TOKEN_SUBPROTOCOL_PREFIX + session.token], headers=origin
        ) as ws:
...
def gather_checks() -> list[Check]:
    ...
    the reds below it."""
    from elizabeth.config import LlmConfig, SttConfig
    from elizabeth.modelspec import select
...
def render(checks: list[Check]) -> int:
    ...
    table = Table(title="elizabeth doctor")
...
def run_doctor(
    ...
    """What `elizabeth doctor` calls. `gather` is the seam a test uses to
