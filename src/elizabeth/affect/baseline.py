...
from elizabeth.affect.features import FEATURE_NAMES, ProsodyFeatures
from elizabeth.config import Elizabeth

log = logging.getLogger(__name__)

STATE_DIR = Path.home() / ".local/state/elizabeth"
...
class SpeakerBaseline:
    ...
    def z_scores(self, features: ProsodyFeatures, cfg: Elizabeth | None = None) -> dict[str, float]:
    ...
    def observe(self, features: ProsodyFeatures, cfg: Elizabeth | None = None) -> bool:
...
class BaselineStore:
    ...
    @classmethod
    def load(cls, path: Path | None = None, cfg: Elizabeth | None = None) -> BaselineStore:
    ...
    def for_device(self, device: str, cfg: Elizabeth | None = None) -> SpeakerBaseline:
        cfg = cfg or Elizabeth()
