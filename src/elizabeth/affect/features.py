...
def _rms(pcm: np.ndarray, cfg: Elizabeth) -> np.ndarray:
...
def _pause_ratio(rms: np.ndarray, cfg: Elizabeth) -> float:
...
def _f0(pcm: np.ndarray, sr: int, cfg: Elizabeth) -> tuple[np.ndarray, float]:
...
def _onset_rate_hz(pcm: np.ndarray, sr: int, cfg: Elizabeth) -> float:
...
def warm(cfg: Elizabeth | None = None, samplerate: int = 16000) -> float:
...
def extract(
    pcm_16k: np.ndarray, cfg: Elizabeth | None = None, samplerate: int = 16000
