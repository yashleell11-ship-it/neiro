...
from elizabeth.llm.prompt import system_message, user_message
from elizabeth.state import NEUTRAL_STATE, ElizabethState, ToolCall, Turn, UserAffect
from elizabeth.tools.registry import ToolRegistry, ToolRejected
from elizabeth.tools.tiers import Tier as ToolTier
...
class TurnResult:
    ...
    transcript: str = ""
    reply: str = ""
    state: ElizabethState = field(default_factory=lambda: NEUTRAL_STATE)
...
class Orchestrator:
    ...
    async def speak(
        self, turn: Turn, chunks: AsyncIterator[str], state_ref: Callable[[], ElizabethState]
    ) -> None:
    ...
    async def run(
        ...
        the caller gets a `TurnResult` with `error` set and Elizabeth says
        ...
        try:
            ...
            def feel(found: ElizabethState) -> None:
                nonlocal state
                state = found
                turn.elizabeth_state = found
            ...
            async def one_round(tools: list[dict] | None) -> tuple[list[str], list[ToolCall]]:
                ...
                if found is not None and turn.elizabeth_state is NEUTRAL_STATE:
                    feel(found)
                if speakable:
                    parts.append(speakable)
