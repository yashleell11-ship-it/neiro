...
`ElizabethState` that drives the face, floored at 0.25 because a completely
...
from elizabeth.emotion.voice import exaggeration_for
from elizabeth.state import NEUTRAL_STATE, ElizabethState, Locality
    ...
    def load(self) -> None:
        ...
        if not self.model_dir.exists():
            raise ChatterboxUnavailable(
                ...
                "`elizabeth fetch-models --only chatterbox`."
            )
        try:
