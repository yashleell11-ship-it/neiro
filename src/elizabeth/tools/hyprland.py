...
payload is built from Elizabeth's own resolved values and validated by
...
from elizabeth.tools.egress import check as egress_check
...
def socket_path() -> Path:
    ...
    if not signature:
        raise HyprlandUnavailable(
            ...
            "Elizabeth's window tools are Hyprland-specific; the voice loop works "
            "without them."
        )
...
class Window:
    """One open window, as Elizabeth sees it.

    `index` is the ONLY handle the model is ever given. It indexes into
    the list Elizabeth produced this turn; Elizabeth resolves it back to
    ...
    def describe(self) -> str:
        """What Elizabeth says about this window out loud."""
...
def dispatch(payload: str) -> str:
    ...
    by Elizabeth from its own resolved values — never interpolated from
...
def focus_window_by_index(index: int, windows: list[Window]) -> str:
    """YELLOW tier. Resolve an index Elizabeth itself produced into an
