...
# `elizabeth.token.<token>`, never the URL. uvicorn writes every handshake's
...
TOKEN_SUBPROTOCOL_PREFIX = "elizabeth.token."
...
# Where `elizabeth talk --browser` listens. Loopback only, always: the token
...
# hold `elizabeth talk` open on exit.
...
def token_from_subprotocols(header: str | None) -> str | None:
    """The token a client offered as `elizabeth.token.<token>`, or None.
...
def face_config(cfg: Elizabeth) -> dict[str, float]:
...
def build_app(
    ...
    app = FastAPI(title="elizabeth", docs_url=None, redoc_url=None, openapi_url=None)
    ...
    async def index() -> HTMLResponse:
        ...
        inject = f"<script>window.ELIZABETH_TOKEN={json.dumps(session.token)};</script>"
        if index_path.is_file():
            ...
            page = (
                "<!doctype html><meta charset=utf-8><title>Elizabeth</title>"
                f"{inject}<p>Elizabeth is running, but web/index.html is missing.</p>"
            )
        return HTMLResponse(page, headers={"Cache-Control": "no-store"})

    @app.websocket("/elizabeth")
...
class Face:
    ...
    `bind()` claims the port synchronously so `elizabeth talk --browser` can
