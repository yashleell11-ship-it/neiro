"""The `elizabeth` command. Stage 0 wires up `doctor`; later stages add
...
app = typer.Typer(
    add_completion=False, help="Elizabeth — an anime character who lives on your machine."
)


@app.callback()
def _main() -> None:
    """Elizabeth — an anime character who lives on your machine.

    A no-op callback. Without it, Typer collapses a single-command app so
    `elizabeth doctor` fails with "unexpected extra argument (doctor)" — this
...
def doctor(
    ...
    from elizabeth.doctor import run_doctor
...
def mic_test(
    ...
    from elizabeth.audio.capture import record
    from elizabeth.audio.playback import play
    ...
    if peak < 1e-4:
        console.print(
            ...
            "isn't muted (elizabeth doctor) and that it's the device you're actually "
            "speaking into."
        )

    out_path = "/tmp/elizabeth-mic-test.wav"
...
@app.command()
def stt(
    wav_path: str = typer.Argument(
        ..., help="WAV file to transcribe, e.g. /tmp/elizabeth-last.wav"
    ),
) -> None:
    """Stage 0 Task 5: transcribe a WAV file with faster-whisper, print
    the result and how long it took.

    Composes with `elizabeth ptt`: record with ptt, transcribe the result
    with `elizabeth stt /tmp/elizabeth-last.wav`.
    """
    import asyncio
    import time
...
def record_set(
    ...
    """
    from elizabeth.config import Elizabeth
    from elizabeth.recordset import run as run_record_set

    raise typer.Exit(code=run_record_set(name=name, count=count, cfg=Elizabeth()))
...
def wer(
    ...
    from elizabeth.evals.wer import load_references, score
    from elizabeth.recordset import DATA_ROOT
    from elizabeth.stt.faster_whisper import FasterWhisperStt
    ...
    if not refs_path.exists():
        console.print(
            ...
            f"Record one first:  elizabeth record-set --name {name} --count 30"
        )
        raise typer.Exit(code=1)
...
def ptt() -> None:
    ...
    recording, q to quit — writes each recording to /tmp/elizabeth-last.wav.
    ...
    """
    from elizabeth.audio.ptt import run as run_ptt
    from elizabeth.config import Elizabeth

    raise typer.Exit(code=run_ptt(Elizabeth()))
...
def talk(
    ...
    Push-to-talk in the terminal, like `elizabeth ptt` — the compositor bind
    is Task 11. Needs llama-server/ollama up (`elizabeth doctor`).
    ...
    """
    from elizabeth.config import Elizabeth
    from elizabeth.talk import run as run_talk

    raise typer.Exit(code=run_talk(Elizabeth(), browser=browser))
...
def affect(
    ...
    from elizabeth.affect.baseline import BaselineStore
    from elizabeth.affect.prosody import ProsodyAffectProvider, band_for, describe
    from elizabeth.config import Elizabeth
...
def say(
    ...
    from elizabeth.llm.emotion_tag import EmotionTagParser
    from elizabeth.state import NEUTRAL_STATE, Turn
...
# the module is run directly (`python -m elizabeth.cli`). It sat mid-file for
