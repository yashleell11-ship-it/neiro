...
~/.local/state/elizabeth/turns.jsonl, so when a turn feels slow the answer
...
from elizabeth.evals.latency import HEADLINE_END, STAGES
from elizabeth.state import Turn

STATE_DIR = Path(os.environ.get("XDG_STATE_HOME", Path.home() / ".local/state")) / "elizabeth"
