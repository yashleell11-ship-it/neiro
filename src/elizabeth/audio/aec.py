...
CONFIG_PATH = Path.home() / ".config/pipewire/pipewire.conf.d/99-elizabeth-aec.conf"
...
#
# suspend-timeout 0 on Elizabeth's nodes matters because PipeWire suspends
...
DROP_IN = """# Elizabeth: echo cancellation for the SPEAKERS profile.
# Written by `elizabeth doctor`; never installed silently.
context.modules = [
  { name = libpipewire-module-echo-cancel
    args = {
      library.name = aec/libspa-aec-webrtc
      node.latency = 1024/48000
      capture.props  = { node.name = "elizabeth_aec_capture"  node.passive = true }
      source.props   = { node.name = "elizabeth_aec_source"   session.suspend-timeout-seconds = 0 }
      sink.props     = { node.name = "elizabeth_aec_sink"     session.suspend-timeout-seconds = 0 }
      playback.props = { node.name = "elizabeth_aec_playback" node.passive = true }
      aec.args = {
